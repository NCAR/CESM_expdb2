...
##    mip_exps = { 'HighResMIP'   : ['control-1950-HR', 'hist-1950-HR', 'highresSST-present-HR', 'highresSST-future-HR'] }
    mip_exps = { 'HighResMIP'   : ['control-1950-LR', 'hist-1950-LR', 'highresSST-present-LR', 'highresSST-future-LR'] }
...
##            exp_tmp = exp.replace('-HR','')
            exp_tmp = exp.replace('-LR','')
            ...
            if not exists:
                ...
                sql = "insert into t2_cmip6_exps (name, description, uid, design_mip, dreq_version) value ('{0}','{1}','{2}','{3}','{4}')".format(exp, db.escape_string(description), uid, mip, version)
                try:
                    print ("Executing sql = {0}".format(sql))
                    cursor.execute(sql)
                    db.commit()
                ...
                if count[0] == 1:
                    # update record with design_mip_id
