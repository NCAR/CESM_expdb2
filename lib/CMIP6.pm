...
sub getCMIP6CaseByID
{
	...
	# get CMIP6 global File Attributes and CMIP6 project values
	($globalAtts, $project) = getCMIP6GlobalFileAtts($dbh, $id, $case{'title'}{'value'});
	...
	while (my $ref = $sth->fetchrow_hashref())
	{
	    ...
	    if ($ref->{'disk_usage'} == 0) {
		$status{$process_name}{'disk_usage'} = "0";
	    }
	    else {
		$status{$process_name}{'disk_usage'} = sprintf("%.0f", $ref->{'disk_usage'}/(1000000));
	    }
...
sub CMIP6publishDSET
{
	...
	while (my $ref = $sth->fetchrow_hashref())
	{
	    ...
	    if( $ref->{'science_id'} > 0 )
	    {
		print STDERR "CMIP6.pm line 870";
...
sub isCMIP6Publisher
{
    ...
    # check if this user_id matches the case science_id or assign_id lead
    $sql = qq(select IFNULL(science_id, 0) as science_id,
              IFNULL(assign_id, 0) as assign_id from t2j_cmip6 
              where $user_id = science_id and case_id = $case_id);
    $sth = $dbh->prepare($sql);
    $sth->execute();
    my ($science_id, $assign_id) = $sth->fetchrow();
    $sth->finish();
    
    if ($is_cmip6_pub || ($science_id > 0 || $assign_id > 0)) {
	return 1;
    }
    return 0;
}
...
sub getCMIP6GlobalFileAtts
{
    ...
    $sql = qq(select e.name as expName, e.description as expDesc, e.activity_id, 
              ...
              IFNULL(j.parentExp_id, 0) as parentExp_id, IFNULL(j.parentCase_id, 0) as parentCase_id, s.value, j.exp_id, e.uid
	...
	$project{'cmip6_variant_info'} = $caseTitle;
	$globalAtts{'variant_info'} = $caseTitle;
